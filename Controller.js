...
// check all files...
  ...
  let result=[];
  //Reading all files from request
    const allInputFiles=req.files
   
    console.log("FileProcessing Count",allInputFiles.length)
    for(let i=0;i<allInputFiles.length;i++){
      console.log("Reading file index",i,allInputFiles[i].path)
    // Read the contents of the uploaded file
    const filePath = allInputFiles[i].path;
    // console.log("FIle Length",filePath[0])
...
res.sendFile(path.join(__dirname, fullFilePath));
  result.push(path.join(__dirname, fullFilePath))
    }


return res.status(200).json({message:"JSON converted successfully",success:true,files:result})
